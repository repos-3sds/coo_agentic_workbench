                ...
                return Object.entries(contractsObj)
                    ...
                    .map(([archetype, value]) => {
                        ...
                        return {
                            ...
                            required_context: Array.isArray(contract['required_context']) ? (contract['required_context'] as unknown[]).map(String) : [],
                            optional_context: Array.isArray(contract['optional_context']) ? (contract['optional_context'] as unknown[]).map(String) : [],
                            excluded_context: Array.isArray(contract['excluded_context']) ? (contract['excluded_context'] as unknown[]).map(String) : [],
                            budget_profile: String(contract['budget_profile'] ?? ''),
                            recent_validations: Array.isArray(contract['recent_validations']) ? contract['recent_validations'] as ContractValidationResult[] : []
                        } as ContextContract;
                    });
            }
        }
